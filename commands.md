...
`help` displays the embed menu in the channel and deletes it after 60 seconds of
inactivity. Using a help category button resets the inactivity timer.
...
| `find map forest` or `/maplibrary forest` | Search only clearly playable, openly licensed RPG maps with creator and license credit. The bot says no match instead of substituting generated or reference maps. Built-in maps require an explicit request such as `find map built in forest`. |
